 ...
 * pixel we process.														  */
//int convolution2D_verti(int posy, int posx, const unsigned char *input, int operator[][3]) {
int convolution2D_verti(int posy, int posx, const unsigned char *input) {
	int i, j, res, tmp;
...
//int convolution2D_horiz(int posy, int posx, const unsigned char *input, int operator[][3]) {
int convolution2D_horiz(int posy, int posx, const unsigned char *input) {
	int i, j, res, tmp;
...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	if (f_golden!= FR_OK) {
		print("File  GOLDEN_FILE  not found\n");
		return XST_FAILURE;
	}
	...
	if (f_out!= FR_OK) {
		print("File  OUTPUT_FILE  not found\n");
		return XST_FAILURE;
	}


	int readBytes=SIZE*SIZE;
	int off =0;
    ...
    for (i=1; i<SIZE-1; i+=1 ) {
    	...
    	for (j=1; j<SIZE-1; j+=1) {
    		//conv_tmp_horiz = convolution2D_horiz(i, j, input, horiz_operator);
    		//conv_tmp_verti = convolution2D_verti(i, j, input, vert_operator);

    		conv_tmp_horiz = convolution2D_horiz(i, j, input);
    		conv_tmp_verti = convolution2D_verti(i, j, input);
...
int main(int argc, char* argv[])
{
	...
	if (result != FR_OK) {
		print("Cannot mount sd\n");
		return XST_FAILURE;
	}
	print("Sobel filter start!\n");
	double PSNR;
	PSNR = sobel(input, output, golden);
	printf("PSNR of original Sobel and computed Sobel image: %g\n", PSNR);
	print("Create the output image with:$convert -depth 8 -size 1024x1024 GRAY:output_sobel.grey output_sobel.jpg\n");
    //cleanup_platform();
