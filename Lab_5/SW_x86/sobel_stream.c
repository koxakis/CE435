...
unsigned char in_cache[SIZE*SIZE];
...
int convolution2D(int posy, int posx, const unsigned char *in_cache, int operator[][3]) {
	int i, j, res;

	res = 0;
	for (i = -1; i <= 1; i++) {
		for (j = -1; j <= 1; j++) {
			res += in_cache[(posy + i)*SIZE + posx + j] * operator[i+1][j+1];
		}
	}
...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden) {
	...
	unsigned int two = 0;
	unsigned int i,j,k;
	unsigned int counter = 0; // used for streaming
	...
	// cache the first two rows and the three first elements from input
	for (i=0; i<2*SIZE; i++) {
		in_cache[i] = input[i];
		counter++;
	}
	...
	for (i=1; i<SIZE-1; i+=1) {
		for (j=1; j<SIZE-1; j+=1) {
			...
			// if we are at a new row read 3 elements otherwise 1.
			if (j == 1) {
				for (k=0; k<3; k++) {
					in_cache[counter] = input[counter];
					counter++;
				}
			}
			else {
			...
			p = convolution2D(i, j, in_cache, horiz_operator) * \
				convolution2D(i, j, in_cache, horiz_operator) + \
				convolution2D(i, j, in_cache, vert_operator) * \
				convolution2D(i, j, in_cache, vert_operator);
