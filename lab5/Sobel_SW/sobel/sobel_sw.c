...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	xil_printf("Output initialy has %d zeros\r\n",zeros);
	...
	if (f_in!= FR_OK) {
		xil_printf("File  INPUT_FILE  not found\r\n");
		return XST_FAILURE;
	}
	Log_File = (char *)GOLDEN_FILE;
	f_golden = f_open(&file2, Log_File,FA_READ);
	if (f_golden!= FR_OK) {
		xil_printf("File  GOLDEN_FILE  not found\r\n");
		return XST_FAILURE;
	}
	...
	if (f_out!= FR_OK) {
		xil_printf("File  OUTPUT_FILE  not found\r\n");
		return XST_FAILURE;
	}
	...
	xil_printf("Just read from golden with start %p\r\n",&golden[0]);
	...
	xil_printf("Measure timer\r\r\n");

	xil_printf("Measured\r\r\n");

	tick = XScuTimer_GetCounterValue(&Timer);

	xil_printf("Ended with %.6lf secs\r\r\n",(double)(-1*tick)/XPAR_PS7_CORTEXA9_0_CPU_CLK_FREQ_HZ);

	xil_printf("Sobel done with ones:%d and twos:%d\r\n",one,two);
	...
	while(writtenBytes!=SIZE*SIZE) {
		...
		if (f_out!=0) {
			xil_printf(" ERROR: f_write2 returned %d\r\r\n",f_out);
			//return XST_FAILURE;
		}
		off+=writtenBytes;
	...
	xil_printf("Just written to output with start %p\r\n",&output[0]);
	xil_printf("Written:%d bytes\r\n",writtenBytes);
...
int main(int argc, char* argv[])
{
	...
	if (result != FR_OK) {
		xil_printf("Cannot mount sd\r\n");
		return XST_FAILURE;
	}
	xil_printf("Sobel filter start!\n");
	double PSNR;
	PSNR = sobel(input, output, golden);
	xil_printf("PSNR of original Sobel and computed Sobel image: %g\r\n", PSNR);
	xil_printf("Create the output image with:$convert -depth 8 -size 1024x1024 GRAY:output_sobel.grey output_sobel.jpg\r\n");
    cleanup_platform();
