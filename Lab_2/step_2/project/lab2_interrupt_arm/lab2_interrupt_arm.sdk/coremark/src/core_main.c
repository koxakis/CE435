...
#error "Please define a way to initialize a memory block."
	...
	// OUR INIT
#ifdef CORE_MULT
	// initialization using coremark functions
	mat_params ar;
	void *memblk = (void *)2048;
	core_init_matrix((ee_u32)SIZE, memblk, 1, &ar);
	ee_printf("N = %d\n", ar.N);
	if (ar.N < 16) {
		ee_printf("A:\n");
		for (i=0; i<ar.N; i++) {
	...
	// OUR code here
	#ifdef CORE_MULT
	// multiplication by coremark functions
	matrix_mul_matrix(ar.N, &ar.C, &ar.A, &ar.B);
	#else
	#if (TILED == 1)
		int m, n, k, tmp;
		ee_printf("WITH BLOCKING\n");
		for (i=0; i < SIZE; i+=BLK_SIZE) {
			for (j=0; j < SIZE; j+=BLK_SIZE) {
				for (k=0; k < SIZE; k++) {
					for (m=i; m < MIN(i+BLK_SIZE, SIZE); m++) {
						tmp = 0;
						for (n=j; n < MIN(j+BLK_SIZE, SIZE); n++) {
							tmp = tmp + A[k*SIZE+n]*B[n*SIZE+m];
						}
						C[k*SIZE+m] += tmp;
					}
				}
